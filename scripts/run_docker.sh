...
docker run --network host -it -v /run/udev:/run/udev -v $(pwd):/Height_Vision_Pi --device /dev/video0:/dev/video0 -e DISPLAY=${DISPLAY} --entrypoint /bin/bash ghcr.io/piotrciosmak/height-vision-pi/$ARCHITECTURE:$VERSION